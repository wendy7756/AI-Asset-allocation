...
class InvReqAccounts(Resource):
    ...
    def post(self, user: User):
        ...
        investDao.insert(Notice('尊敬的用户：您已创建一个新的资产账户，请尽确认交易', user.username))
...
class InvReqAccountOverview(Resource):
    ...
    def get(self, user: User, invreq_id: str):
        account = RestDao().getInvestRequirementByKey(invreq_id)
        dict_return = {"accuRevenue": account.accu_revenue, "predRorYear": account.pred_revenue_year,
...
class InvReqAccountMatchining(Resource):
    ...
    @ns.response(200, "股票、债券、商品市场的配比详情", ns.model("股票、债券、商品市场的配比详情", {
        "stockMatching": fields.List(fields.Nested(matching), description="股票的配比信息"),
        "bondMatching": fields.List(fields.Nested(matching), description="债券的配比信息"),
        "productionMatching": fields.List(fields.Nested(matching), description="商品市场的配比信息"),
    }))
    @login_require()
    def get(self, user: User, invreq_id: str):
        # account = RestDao.getInvestRequirementByKey(invreq_id)
        ...
        dist_return = {"stockMatching": 100 * match.stock / sum, "bondMatching": 100 * match.bonds / sum,
                       "productionMatching": 100 * match.goods / sum}
...
class InvReqReallocationLog(Resource):
    ...
    def get(self, user: User, invreq_id: str):
        ...
        for r in records:
            ...
            ret.append(
                {'tranDate': str(r.time).split(' ')[0], 'perOfStock': a2, 'changeOfStock': a2 - a1, 'perOfBond': b2,
                 'changeOfBond': b2 - b1, 'perOfProduct': c2, 'changeOfProduct': c2 - c1})
        return ret, 200
        pass
