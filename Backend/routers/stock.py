...
class StockOverviewApi(Resource):
    ...
    def get(self, invreq_id: str):
        ...
        return {'startDate': str(investDao.get_one_invest(invreq_id).start_time).split(' ')[0],
                'currentVolume': matching.stock,
                'currentRatio': round(matching.stock / (matching.stock + matching.bonds + matching.goods + 0.000001), 2)}, 200
        pass
