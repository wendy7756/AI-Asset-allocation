...
def get_data():
    ...
    timer = Timer(delta_seconds_per_minute, get_data)
    timer.start()
...
def update_allocation():
    timer = Timer(20, update_allocation)
    timer.start()
    ...
    for invest in all_invest:
        ...
        if not invest.defaultConfirm:
            investDao.insert(Notice('尊敬的用户：您的资产配置方案已产生变更，请尽快确认交易', invest.username))
        else:
