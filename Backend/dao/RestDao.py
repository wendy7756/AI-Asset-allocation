            ...
            for s in position_arr:
                if r.request_id == s.request_id and r.code == s.code:
                    ...
                    if r.quantity > s.quantity:
                        ...
                        # 买入
                        temp = TransactionRecord(r.code, r.price, r.quantity - s.quantity, r.time, expense, r.type, invreq_id)
                        temp.is_done = False
                        temp.name = r.name
                    ...
                    if r.quantity < s.quantity:
                        ...
                        # 卖出
                        temp = TransactionRecord(r.code, r.price, s.quantity - r.quantity, r.time, expense, r.type, invreq_id)
                        temp.is_done = False
                        temp.name = r.name
    ...
    def generateRecordsAndChange(self, invreq_id):
        ...
        for i in transactionRecords:
            ...
            if i.type == Mode.GOODS:
                i.matching = (i.price * i.quantity) / (r_record.goods_after + 0.000001)
            elif i.type == Mode.BOND:
                i.matching = (i.price * i.quantity) / (r_record.bonds_after + 0.000001)
            elif i.matching == Mode.STOCK:
                i.matching = (i.price * i.quantity) / (r_record.stock_after + 0.000001)
            investDao.insert(i)
