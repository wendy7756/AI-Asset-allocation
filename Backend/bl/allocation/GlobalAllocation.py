                ...
                if min_risk > anticipated_risk:
                    ...
                    if w[1] > 1:
                        ...
                        if len(goods_strategy) == 2:
                            # 原油期货
                            advice = Advice(goodsDao.get_future_code_of_crude_oil(), goods_strategy[0], 0, Mode.GOODS)
                            result.append(advice)
                            # 原油现货
                            advice = Advice(goodsDao.get_spot_code_of_crude_oil(), goods_strategy[1], 0, Mode.GOODS)
                            result.append(advice)
                        print('goods')

                    # 债券市场部分
                    if w[2] + w[3] > 1:
                        bonds_strategy1 = BondsBl().ged_interestbonds(w[3], r[3], min_risk, year)
                        print('bond1')
                        bonds_strategy2 = BondsBl().get_creditbonds(w[2], r[2], min_risk, year)
                        print('bond2')
