...
class StockBl(object):
    ...
    def calculate_quarter_regression(self, big_growg, big_worthg, mid_growg, mid_worthg, small_growg,
        ...
        for i in range(np.math.floor(big_growg.__len__() / 90)):
            ...
            power = linreg.coef_
            power.append(linreg.intercept_)
    ...
    def get_achievement(self, req_id):
        ...
        for j in range(stocks.__len__()):
            ...
            form_data.append({
                ...
                'maxReturn': sum(stock_revenueg),  # 超额最大回撤率：百分比
    ...
    def get_reason(self, req_id):
        ...
        style_ratio, styleg, r_2 = self.calculate_quarter_regression(big_growg, big_worthg, mid_growg, mid_worthg,
                                                                     ...
                                                                     small_worthg, myfundg)
        style_timing = []
        for i in range(base_300g.__len__()):
        ...
        time_over = []
        time_below = []
        for i in range(base_ratio.__len__()):
            over_num = 0
            below_num = 0
            for j in range(base_ratio[i].__len__()):
                if base_ratio[i][j] < style_ratio[i][j]:
                    over_num += 1
                elif base_ratio[i][j] > style_ratio[i][j]:
                    below_num += 1
        ...
        style_config_form = [{
            ...
            'yearRevenue': style_timingr,
        }, {
            'style': "风格择时贡献",
            'winRate': style_choosew,
            'yearRevenue': style_chooser,
        }]
        style_explain_form = []
        for i in range(dateq.__len__()):
        ...
        prefer_and_contr_list.append({
            'style': "平均持仓",
            'largeGrowth': sum_style_ratios[0],
            'largeValue': sum_style_ratios[1],
            'middleGrowth': sum_style_ratios[2],
            'middleValue': sum_style_ratios[3],
            'smallGrowth': sum_style_ratios[4],
            'smallValue': sum_style_ratios[5]
        })
        prefer_and_contr_list.append({
            'style': "平均超额收益率",
            'largeGrowth': np.average(big_growg) / 630 * 365,
            'largeValue': np.average(big_worthg) / 630 * 365,
            'middleGrowth': np.average(mid_growg) / 630 * 365,
            'middleValue': np.average(mid_worthg) / 630 * 365,
            'smallGrowth': np.average(small_growg) / 630 * 365,
            'smallValue': np.average(small_worthg) / 630 * 365
        })
        prefer_and_contr_list.append({
            'style': "超配次数",
        ...
        return {
            ...
            'preferAndContrList': prefer_and_contr_list
        }
    ...
    def get_scene(self, req_id):
        ...
        volatility_market_up_and_big_up = np.std(market_up_and_big_up)
        volatility_market_down_and_big_up = np.std(market_down_and_big_up)
        volatility_market_up_and_small_up = np.std(market_up_and_small_up)
        volatility_market_down_and_small_up = np.std(market_down_and_small_up)
        ...
        sortino_ratio_market_up_and_big_up = np.std(market_up_and_big_up, ddof=1)
        sortino_ratio_market_down_and_big_up = np.std(market_down_and_big_up, ddof=1)
        sortino_ratio_market_up_and_small_up = np.std(market_up_and_small_up, ddof=1)
        sortino_ratio_market_down_and_small_up = np.std(market_down_and_small_up, ddof=1)
        ...
        line_regression.fit(fluctuationf, getf)
        line_pred = line_regression.predict(fluctuationf)
        # 曲线拟合
        ...
        market_route_form = [{
            'index': '周期数',
            'obviousUp': market_up_and_big_up_num,
            'waveryUp': market_down_and_big_up_num,
            'obviousDown': market_up_and_small_up_num,
            'waveryDown': market_down_and_small_up_num
        }, {
            'index': '场景收益占比',
            'obviousUp': market_up_and_big_up_rate,
            'waveryUp': market_down_and_big_up_rate,
            'obviousDown': market_up_and_small_up_rate,
            'waveryDown': market_down_and_small_up_rate
        }, {
            'index': '波动率',
            'obviousUp': volatility_market_up_and_big_up,
            'waveryUp': volatility_market_down_and_big_up,
            'obviousDown': volatility_market_up_and_small_up,
            'waveryDown': volatility_market_down_and_small_up
        }, {
            'index': '夏普比率',
            'obviousUp': sharpe_ratio_market_up_and_big_up,
            'waveryUp': sharpe_ratio_market_down_and_big_up,
            'obviousDown': sharpe_ratio_market_up_and_small_up,
            'waveryDown': sharpe_ratio_market_down_and_small_up
        }, {
            'index': '最大回撤率',
            'obviousUp': max_back_market_up_and_big_up,
            'waveryUp': max_back_market_down_and_big_up,
            'obviousDown': max_back_market_up_and_small_up,
            'waveryDown': max_back_market_down_and_big_up
        }, {
            'index': 'Sortino比率',
            'obviousUp': sortino_ratio_market_up_and_big_up,
            'waveryUp': sortino_ratio_market_down_and_big_up,
            'obviousDown': sortino_ratio_market_up_and_small_up,
            'waveryDown': sortino_ratio_market_down_and_small_up
        }, {
            'index': 'Calmar比率',
            'obviousUp': calmar_ratio_market_up_and_big_up,
            'waveryUp': calmar_ratio_market_down_and_big_up,
            'obviousDown': calmar_ratio_market_up_and_small_up,
            'waveryDown': calmar_ratio_market_down_and_small_up
        }]
        sensibility_chart = []
        for i in range(fluctuationf.__len__()):
            sensibility_chart.append({
                'fluctuation': round(fluctuationf[i], 2),
                'lineRevenue': round(line_pred[i], 2),
